...
if (!$municipality_id) {
    die("No municipality set for this admin.");
}
...
/* ---------- Patients to monitor (scoped + optional search) ---------- */
$sql = "
    SELECT 
        p.*,
        b.name AS barangay
...
function calc_edd($r) {
    if (!empty($r['edd'])) {
        return date('M d, Y', strtotime($r['edd']));
    }
    if (!empty($r['lmp'])) {
...
function risk_level($r) {
    if (!empty($r['risk_level'])) return strtolower($r['risk_level']); // honor explicit column if exists
...
<style>
    ...
    *{ box-sizing:border-box }
    ...
    /* ===== Left sidebar (same style as dashboard/view_pregnancies) ===== */
    .leftbar{
        width: var(--sidebar-w);
        background:#ffffff;
    ...
    .risk-normal  { color:#0ca678; }  /* green */
    .risk-caution { color:#f59f00; }  /* amber */
    .risk-high    { color:#e03131; }  /* red */
...
<div class="layout">
    ...
    <aside class="leftbar">
        ...
        <nav class="nav flex-column gap-1">
            ...
            <a class="nav-link" href="barangay_pregnancies.php"><i class="bi bi-building"></i> Brgy. Health Centers</a>
                ...
                <table class="table align-middle mb-0">
                    ...
                    <?php if ($rows && $rows->num_rows > 0): ?>
                        <?php while ($r = $rows->fetch_assoc()):
                            ...
                            $required   = 3; // adjust if you have per-patient plan
                            $next_sched = !empty($r['next_schedule']) ? date('M d, Y', strtotime($r['next_schedule'])) : 'N/A';

                            $risk      = risk_level($r); // normal|caution|high
